...
class TensorboardSummary(object):
    ...
    def visualize_image_mulitview(self, writer, dataset, image, target, output_single, output_multiview, global_step):
        ...
        writer.add_image('Train Predicted_label_singleview', grid_image, global_step)
        grid_image = make_grid(decode_seg_map_sequence(torch.max(output_multiview[:4], 1)[1].detach().cpu().numpy(),
                                                       dataset=dataset), 4, normalize=False, range=(0, 255))
        writer.add_image('Train Predicted_label_multiview', grid_image, global_step)
        grid_image = make_grid(decode_seg_map_sequence(torch.squeeze(target[:4], 1).detach().cpu().numpy(),
                                                       dataset=dataset), 4, normalize=False, range=(0, 255))
        writer.add_image('Train Groundtruth_label', grid_image, global_step)

    def visualize_validation_mulitview(self, writer, dataset, image, target, output_single, output_multiview, global_step):
        grid_image = make_grid(image[:4].clone().cpu().data, 4, normalize=True)
        writer.add_image('Validation Image', grid_image, global_step)
        grid_image = make_grid(decode_seg_map_sequence(torch.max(output_single[:4], 1)[1].detach().cpu().numpy(),
                                                       dataset=dataset), 4, normalize=False, range=(0, 255))
        writer.add_image('Validation Predicted_label_singleview', grid_image, global_step)
        grid_image = make_grid(decode_seg_map_sequence(torch.max(output_multiview[:4], 1)[1].detach().cpu().numpy(),
                                                       dataset=dataset), 4, normalize=False, range=(0, 255))
        writer.add_image('Validation Predicted_label_multiview', grid_image, global_step)
        grid_image = make_grid(decode_seg_map_sequence(torch.squeeze(target[:4], 1).detach().cpu().numpy(),
                                                       dataset=dataset), 4, normalize=False, range=(0, 255))
        writer.add_image('Validation Groundtruth_label', grid_image, global_step)
